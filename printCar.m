function printCar(initral_x,initral_y,stick_long,stick_th,car_angle)
    xp=[initral_x initral_x+stick_long*sin(stick_th-car_angle)]; % 桿子
    yp=[initral_y initral_y+stick_long*cos(stick_th-car_angle)];
    plot(xp,yp,'Color',[1,0,0])
    hold on

    the=0:0.01:2*pi;% 桿子頂端球
    x=initral_x+stick_long*sin(stick_th-car_angle)+(1/10)*stick_long*cos(the); 
    y=initral_y+stick_long*cos(stick_th-car_angle)+(1/10)*stick_long*sin(the); 
